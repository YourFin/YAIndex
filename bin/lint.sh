...
FIX=false
FIX_SAFE=false
JS=0 # Using numbers so we can do arithmetic, because fuck bash
RUBY=0
ELM=0
...
if (( JS || ! ( ELM || RUBY ))) ; then # I can't figure out how to move this
                                     # expression into a variable. Grrr.
...
## Elm
ELMFMT_COMMAND="yarn run elm-format ."
if (( ELM || ! ( JS || RUBY ))) ; then
    echo "Running elm-format (elm)..."
    if $FIX ; then
        eval "$ELMFMT_COMMAND --yes" \
            || EXIT_STATUS="1"
    else
        eval "$ELMFMT_COMMAND --validate" \
            || EXIT_STATUS="1"
    fi
fi
